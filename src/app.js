...
export default class Application {
	...
	static UpdateTable() {
		...
		const formatter = (operationType) => {
			return (operationTime, row) => {
				...
				let formatted = `<span style='color: #${operationTime ? '000' : 'bbb'}'>${secondsToTimeFormat(operationTime)}</span>`

				if (operationType && toolName !== '#Total'){
					formatted += `<div style='color: #${tool.LifeSpan[operationType] ? '43A047' : '43A04766'}'>${secondsToTimeFormat(tool.LifeSpan[operationType] * 60)}</div>`
				}

				return html(formatted);
			}
		}
		// this overwirte default sort function to prevent from sum row (last augmented row) to change position
		const sort = {
			compare: (a, b) => {
				// last augmented row values start with # so we know it last sum rown and avoid sorting to affect it
				if (a[0] === '#' || b[0] === '#') {
					return 0;
				}
				if (a === 'N/D') return -1;
				if (Number(a.toString().replace(/,/g, '')) > Number(b.toString().replace(/,/g, ''))) {
					return -1;
				} else {
					return 1;
				}
		...
		Application.Grid = new Grid({ 
			...
			columns: [{
				...
				formatter: (name) => {
					...
					if (formatted !== 'Total') {
						formatted += `<div style='color: #B71C1C;font-size: 12px;'>Price: ${Application.Data.GetToolByName(name).Price}</div>`
					}
					return html(formatted);
				},
				sort
				...
				name: 'Diameter',
				formatter: (diameter) => {
					return diameter === '#Diameter' ? 'Diameter' : html(`${diameter}<b style='font-size: 9px'>mm</b>`)
				},
				sort
			}, {
				name: 'Operation Time',
			...
			data: () => {
				return new Promise(resolve => {
					setTimeout(() =>
						resolve(Application.Data.ToolsList.map(tool => {
							return [
							tool.Name, tool.Diameter,
							...Object.values(tool.OperationTime),
							tool.Supply === '#Supply' ? tool.Supply : tool.Supply ? (tool.Supply * Application.Scale).toFixed(4) : 'N/D', // supply
							tool.Supply ? (tool.Name === '#Total' ? '#' : '') + Math.floor((tool.Supply === '#Supply' ? 1 : tool.Supply) * Application.Scale * (tool.Price + '').replace(/,/g, '')).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : 'N/D' // cost
							]
						})), 1000);
				});
			},
		...
		Application.State.Sort.Timer = setTimeout(() => {
			if (Application.State.Sort.Id) {
				...
				for (let i = 0; i <= Application.State.Sort.Type;i++) {
					document.querySelector(`[data-column-id="${Application.State.Sort.Id}"]`).dispatchEvent(clickEvent);
				}
			}
	...
	static ShowToolDataDialog(toolName) {
		...
		const tool = Application.Data.ToolsList[Application.Data.GetToolIndexByName(toolName)];
		...
		for (const operationType of formData.keys()) {
			...
			input.value = tool.LifeSpan[operationType] || (restoredTool ? restoredTool.LifeSpan[operationType] : 0);
		}
		document.querySelector('#tool-price').value = tool.Price || (restoredTool ? restoredTool.Price : 0);
	}
	...
	static SaveToolData() {
		...
		const tool = Application.Data.ToolsList[Application.Data.GetToolIndexByName(toolName)];
		Application.Data.ToolsList.at(-1).Price -= tool.Price.toString().replace(/,/g, '') * tool.Supply;
		tool.Supply = 0;
