...
export class Report {
	...
	constructor(template) {
		...
		if (template) {
			this.ExtactToolNames();
			this.ExtractOperationTime();
		}
	}
	// scrap the input template and search for tool names, the output is stored in `ToolsList`
	ExtactToolNames() {
		const idContainerTags = this.Template.querySelectorAll(SELECTORS['TOOL-ID']);
		const nameContainerTags = this.Template.querySelectorAll(SELECTORS['TOOL-NAME']);
		const diameterContainerTags = this.Template.querySelectorAll(SELECTORS['TOOL-DIAMETER']);
		for (let i = 0;i < idContainerTags.length;i++) {
			this.ToolsList.push(
				new Tool(idContainerTags[i].innerText, nameContainerTags[i].innerText, Number(diameterContainerTags[i].innerText))
				);
		}
	}
	// for each tool in `ToolsList` gather operetion time
	ExtractOperationTime() {
		const typeContainerTags = this.Template.querySelectorAll(SELECTORS['OPERATION-TYPE']);
		const timeContainerTags = this.Template.querySelectorAll(SELECTORS['OPERATION-TIME']);
		const idContainerTags = this.Template.querySelectorAll(SELECTORS['OPERATION-TOOL-ID']);
		if (!(typeContainerTags.length !== 0 && typeContainerTags.length === timeContainerTags.length && timeContainerTags.length === idContainerTags.length)) {
			this.Healthy = false;
			return;
		}
  ...
  static Merge(reports) {
  	...
  	for (const report of reports) {
          ...
          for (const [operationType, operationTime] of Object.entries(tool.OperationTime)) {
          ...
          mergedReport.ToolsList.push(new Tool(tool.ID, tool.Name, tool.Diameter));
          // Copy operation times
          for (const [operationType, operationTime] of Object.entries(tool.OperationTime)) {
          	mergedReport.ToolsList[mergedReport.ToolsList.length - 1].OperationTime[operationType] = operationTime;
          }
          mergedReport.ToolsList.at(-1).ParticipationCount += tool.ParticipationCount;
        }
      }
...
export class Tool {
	...
	OperationTime = {}
	// its duribility in each operation time (in minutes)
	LifeSpan = {}
	// price for this tool
	Price = 0
